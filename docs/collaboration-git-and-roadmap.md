...
- **目标**：**六次独立操作**，每次确定一爻，自下而上累积（初爻→上爻）；最终提交给后端的 **`result` 字符串** 与后端一致（**上爻在前、初爻在后**；前端 `lines[0]` 为上爻、`lines[5]` 为初爻，`linesToResultString` 直接 join，勿 reverse）。  
...
- [x] **M2 起卦六次摇**：状态机（逐爻 → 预览 → 排盘）、爻序与 `result` 一致、页脚文案与行为一致。
- [x] **M3 手动录入**：六位选择、校验、`cast` 同路径；`PROJECT.md` 用户流程已补充。
